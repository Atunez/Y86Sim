...
uint64_t Tools::getByte(uint64_t source, int32_t byteNum) {
	return getBits(source, byteNum * 8, (byteNum * 8) + 7);   
}
...
uint64_t Tools::getBits(uint64_t source, int32_t low, int32_t high) {
	if(high > 63 || low < 0 || high - low < 0) {
		return 0;
	}

	uint64_t full = 0xffffffffffffffff;
	uint64_t answer = full >> (63 - (high - low))  & source >> low;
	return answer;
}
...
uint64_t Tools::setBits(uint64_t source, int32_t low, int32_t high) {
	...
	uint64_t full = 0xffffffffffffffff;
	uint64_t editfull = full >> low;
	editfull = editfull << (63 - (high - low));
	editfull = editfull >> (63 - high);
	uint64_t answer = editfull | source;
	return answer;
}
...
uint64_t Tools::copyBits(uint64_t source, uint64_t dest, 
	...
	if(dstlow > 63 || dstlow < 0 || srclow > 63 || srclow < 0 || 
        dstlow+length > 64 || dstlow+length < 0 || srclow + length > 64 || 
        srclow + length < 0) {
		return dest;
	}

	uint64_t needBits = getBits(source, srclow, srclow + length - 1) << dstlow;
	uint64_t workBits = clearBits(dest, dstlow, dstlow + length - 1);
	uint64_t answer = workBits | needBits;
...
uint8_t Tools::sign(uint64_t source) {
	uint64_t answer = clearBits(source, 0, 62);
	return answer >> 63;
}
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2) {
	uint64_t answer = op1 + op2;
	return !((sign(answer) == sign(op1)) || (sign(answer) ==  sign(op2)));
}
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2) {
   ...
   uint8_t sign3 = sign(diff);

   //A is op2 == sign2 and B is op1 == sign1

   if( (sign2 == 0 && sign1 == 1 && sign3 == 1) || (sign2 == 1 && sign1 == 0 && sign3 == 0) ) {
	return true;
   }
	return false;
}
